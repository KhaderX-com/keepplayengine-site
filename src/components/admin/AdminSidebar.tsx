...
export default function AdminSidebar({ activeSessions = 0, isMobileMenuOpen, onCloseMobileMenu }: AdminSidebarProps) {
    ...
    return (
        <>
            ...
            <aside
                className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-gray-900 via-gray-900 to-gray-800 border-r border-gray-700 transition-all duration-300 z-[50] 
            ...
            >
                ...
                <nav className="px-2 lg:px-3 py-4 lg:py-6 space-y-1 overflow-y-auto h-[calc(100vh-4rem)] lg:h-[calc(100vh-5rem)] overscroll-contain">
                    {navigation.map((item) => (
                        <div key={item.name}>
                            {/* Main Admin Item - Touch Friendly */}
                            <button
