...
import { AdminDAL, TasksDAL, LabelsDAL, getUserClient } from "@/lib/dal";
...
export default async function AdminPage() {
    ...
    // Fetch stats server-side via DAL (no API round-trip)
    let stats: Stats = {
        totalUsers: 0,
        activeSessions: 0,
        recentLogins: 0,
    ...
    try {
        const client = await getUserClient(user.id, userRole);
        const [statsRes, tasksRes, labelsRes] = await Promise.all([
            AdminDAL.getDashboardStats(client),
            TasksDAL.list(client, {}),
            LabelsDAL.list(client),
        ]);

        if (statsRes.data) {
            // RPC returns snake_case keys — map them to the Stats interface
            const raw = statsRes.data as Record<string, number>;
            stats = {
                ...stats,
                totalUsers: raw.total_admins ?? raw.totalUsers ?? stats.totalUsers,
                activeSessions: raw.active_sessions ?? raw.activeSessions ?? stats.activeSessions,
                recentLogins: raw.recent_logins_24h ?? raw.recentLogins ?? stats.recentLogins,
                failedAttempts: raw.failed_attempts_24h ?? raw.failedAttempts ?? stats.failedAttempts,
            };
        }
        if (tasksRes.data) {
        ...
        if (labelsRes.data) {
            stats.labels = labelsRes.data.length;
        }
    } catch (error) {
